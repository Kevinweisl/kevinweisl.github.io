...
## 2026-09-10 — 一根欄只回答一個問題

**Context**: Experience 頁的 rail 頂端放了 `9 Roles`（項目總數），下面則是每筆經歷的期間。
Kevin 問「9 ROLES 是什麼意思？可以拿掉嗎」。

**Mistake**: 那根欄的職責是「定位」——它回答 *when*。`9 Roles` 回答的是 *how many*，
是同一根欄裡唯一一個換了單位的東西。它自己不是錯的資訊，但放在那個位置就變成雜訊：
讀者由上往下掃日期時，第一格不是日期。需要被問「這是什麼意思」的標籤，本身就是答案。

**Rule**: 把東西放進一根欄之前，先講出這根欄回答的**那一個**問題（when / which / where in

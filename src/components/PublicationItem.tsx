...
const PublicationItem: React.FC<Publication> = ({
  ...
  const highlightedAuthors = authors.replace(
    /Sheng-Lun Wei\*?/,
    (match) => `<strong class="gradient-text font-semibold" style="-webkit-text-fill-color:transparent">${match}</strong>`
  );
  ...
  return (
    ...
    >
      ...
      {showAbstract && abstract && (
        <div className="mt-4 p-3 bg-[var(--bg-primary)] rounded-[var(--radius)] border border-[var(--border)] text-sm text-[var(--text-primary)]">
          ...
          <button
            ...
            className="text-xs text-[var(--accent)] hover:underline mt-2 inline-flex items-center cursor-pointer"
          >
      ...
      {showBibtex && bibtex && (
        <div className="mt-4 p-3 bg-[var(--bg-primary)] rounded-[var(--radius)] border border-[var(--border)] text-sm text-[var(--text-primary)]">
          ...
          <pre className="bg-[var(--bg-card)] p-2 rounded-[var(--radius)] text-xs border border-[var(--border)] font-mono whitespace-pre-wrap break-words text-[var(--text-body)]">
            <code>{bibtex}</code>
          </pre>
          <button
            onClick={() => navigator.clipboard.writeText(bibtex || '')}
            className="text-xs bg-[var(--accent-light)] hover:opacity-80 text-[var(--accent)] px-2 py-0.5 rounded-[var(--radius)] mt-2 mr-2 transition-colors cursor-pointer"
          >
            Copy
          </button>
          <button
